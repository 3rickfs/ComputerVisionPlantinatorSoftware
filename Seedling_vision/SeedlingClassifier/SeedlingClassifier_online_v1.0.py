...
while True:
    ...
    if plcInstruction == lsmodb.PLC_PROCODD_INST:
        ...
        if CV_system_switch == "SysP":
            ...
            rgbGUI,chopSeedlings = cvSystem.processSeedlings("odd",CV_MODE) # IM CHANGING CONSTANTLY THIS PARAMETER


            rgbGUI,chopSeedlings = cvSystem.processSeedlings("even",CV_MODE)
            ##############################################################################################
            for i in range(len(chopSeedlings)):
               try:
                 name = "dashboard_images/planta{number}".format(number = i + 1)
                 cv2.imwrite(name + ".jpg", chopSeedlings[i])
               except:
                 question = cv2.imread("dashboard_images/question.jpeg")
                 cv2.imwrite("dashboard_images/planta{number}.jpg".format(number = i + 1),question)
                 print("NO EXISTE ESTA PLANTA")
